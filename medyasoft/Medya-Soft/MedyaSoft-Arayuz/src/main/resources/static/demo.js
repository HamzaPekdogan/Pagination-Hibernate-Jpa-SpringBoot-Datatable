$(function () {
  if (window.JpegCamera) {
    ...
    var add_snapshot = function(element) {
      ...
      var $container = $("#snapshots").html(element);
      var $camera = $("#camera");
...
function saveImage() {
  var canvasPhoto = document.getElementsByTagName('canvas');
    var canvasData = canvasPhoto[0].toDataURL();
    var base64ImageContent = canvasData.replace(/^data:image\/(png|jpg);base64,/, "");
    var blob = base64ToBlob(base64ImageContent, 'image/png');
    var oMyForm = new FormData();
    oMyForm.append("imgData", blob);
    $.ajax({
        url: 'userPhotoSave',
        method: 'post',
        processData: false,
...
function saveStudent() {
  $("#studentAgeName").val((new Date($("#studentAge").val())).getTime());
    var oMyForm = new FormData(document.getElementById('saveStudent'));
    $.ajax({
        url: 'userAddPost',
        method: 'post',
        processData: false,
